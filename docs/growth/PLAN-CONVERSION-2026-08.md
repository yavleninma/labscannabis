...
Внутри `.boutique` цепочек нет, всё как задумано. Но домен `labscannabis.com` к
проекту Vercel не подключён вовсе: два правила с `has: host` в `vercel.json` не
срабатывают никогда, а сам домен отвечает 404 со страницы Wix. Любая внешняя
ссылка на старый домен, любой человек, набравший его руками, и весь
исторический вес уходят в 404.

**ownerTask O-17 (новый, приоритет высокий):** подключить `labscannabis.com` к
проекту Vercel и направить DNS туда. До этого правила апекса в `vercel.json` —
мёртвый код, а не страховка.
