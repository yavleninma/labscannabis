...
- **Яндекс.Вебмастер** (аккаунт владельца, сайт в списке): запрошен переобход
  `sitemap-index.xml`, чтобы Яндекс перечитал индекс и подобрал четыре новые
  карты вместо `sitemap-0.xml` (его больше нет, он отдаёт 404 и выпадет сам).
  Отдельно отправлены на переобход 17 URL: девять новых вопросных страниц на
  th/ar/zh/ko/ja и восемь изменившихся русских страниц. Дневной лимит — 150.
- **Google Search Console — сделать НЕ УДАЛОСЬ.** В браузере залогинен
  `mad1gds@gmail.com`, и у этого аккаунта нет доступа к ресурсу
  `labscannabis.boutique`: панель отдаёт «У вас нет доступа к этому ресурсу», а
  список ресурсов аккаунта пуст. Ресурс заведён на другом аккаунте Google.
  **ownerTask:** войти под тем аккаунтом и в «Файлы Sitemap» добавить
  `sitemap-pages.xml`, `sitemap-strains.xml`, `sitemap-questions.xml`,
  `sitemap-getting-here.xml`, а строку `sitemap-0.xml` удалить. Либо выдать
  доступ уровня «Полный» аккаунту, который залогинен в агентском браузере.

### 12.10 Telegram включён, LINE — нет

Владелец подтвердил 31.08.2026, что оба аккаунта заведены на том же номере, что и
WhatsApp, и что ссылка на Telegram открывает у него приложение с нужным
аккаунтом. Telegram включён (`telegramEnabled: true`). LINE остался выключенным,
и разница здесь не в степени готовности, а в том, что один из двух тестов
доказательный, а другой — нет.

**Telegram — включён.** `t.me/+<цифры>` отдаёт настоящую контактную страницу:
«Chat with +66 6 6080 6784» и кнопка OPEN CHAT, которая передаёт номер в
приложение. Остаются две вещи, которые тест владельца покрыть не может, и обе
записаны в `src/data/site.ts`:

1. Приложение постороннего человека разрешает номер, только пока в настройках
   приватности аккаунта «Кто может найти меня по номеру телефона» стоит «Все».
   Поменяется настройка — кнопки на сайте тихо перестанут работать, и ни одна
   проверка репозитория этого не заметит.
2. Ссылка `t.me/+<цифры>` двусмысленна: Telegram читает `+` сначала как хеш
   приватного приглашения, поэтому СЕРВЕРНОЕ превью ссылки — `og:title = "Join
   group chat on Telegram"` с пустым описанием. Это то, что видит человек, когда
   нашу ссылку пересылают в мессенджере.

Обе проблемы снимает `@username`: он не зависит от настройки приватности и даёт
нормальное превью. Это следующее, о чём стоит попросить владельца, — но кнопка
работает и без него.

**LINE — выключен, и номер телефона это не чинит.** Владелец сообщил, что
страница LINE показывает QR-код. Показывает — но это ничего не значит:
`line.me/R/ti/p/<токен>` рисует QR для ЛЮБОЙ строки и никогда не проверяет её.
Контрольный опыт 31.08.2026 — три запроса подряд:

| Токен | Что отдала страница |
|---|---|
| `660806784` (наш) | «Отсканируйте QR-код, чтобы добавить друга» |
| `zzzz-not-a-real-line-id-9999` | тот же экран, тот же размер QR |
| `~labscannabis` | тот же экран, тот же размер QR |

Ни имени аккаунта, ни аватара, `og:title` — общий «Add LINE friend». То есть
десктопная страница LINE — статическая заглушка, и по ней нельзя отличить
рабочую ссылку от мусора.

Причина глубже формата: `line.me/R/ti/p/` принимает идентификатор — `~<lineId>`
для личного ID или `@<id>` для официального аккаунта, либо короткую ссылку
`lin.ee/XXXX`. **Ссылки для добавления по номеру телефона у LINE нет вообще**:
телефонная книга работает только внутри приложения и только между людьми,
которые уже сохранили друг друга. Поэтому «тот же номер» здесь не помогает
ни в каком виде.

**Нужно от владельца для LINE (2 минуты в приложении):** LINE → Настройки →
Профиль → ID: включить «Разрешить поиск по ID» и прислать сам ID. Тогда ссылка
станет `https://line.me/ti/p/~<ID>`. Либо, что лучше для магазина, завести LINE
Official Account и прислать его ссылку `https://lin.ee/XXXX` — она не зависит от
личного аккаунта и переживает смену сотрудника у прилавка.
