...
## 12. Волна 3 — исполнено 2026-08-31: скорость, граф, сущность, пять языков

Все числа ниже — замеры на этом дереве до и после правки, а не оценки.

### 12.1 Ноль блокирующих отрисовку запросов

LCP-элемент на всех страницах — заголовок H1 (в `dist` ноль JS-бандлов и ноль
`<img>`), значит LCP ≈ FCP и упирается в число блокирующих запросов в `<head>`.
Их было два, стало ноль.

Замер сжатого ответа `fonts.googleapis.com/css2` при одинаковом User-Agent —
цена оказалась резко несимметричной по локалям:

| Локаль | Байт | Объявлений `@font-face` |
|---|---|---|
| en, ru | 742 | 28 |
| th | 1 261 | 37 |
| ar | 2 425 | 43 |
...
Страница целиком весит 18,5 КБ по проводу. То есть на zh/ja/ko первая отрисовка
ждала кросс-доменный файл в пять раз тяжелее самой страницы — и это ещё до
первого байта шрифта. Причина не в жадности Google: CJK-набор режется на три-
четыре сотни срезов по `unicode-range`, и манифест этих срезов и есть тело
ответа.

Сделано:

- `scripts/fetch-fonts.mjs` (`npm run gen:fonts`, вручную, результат в
  репозитории) забирает шесть вариативных `woff2` — Inter latin / latin-ext /
  cyrillic / cyrillic-ext, Noto Sans Thai, Noto Sans Arabic, всего 362 КБ — и
  пишет `src/styles/fonts.css` с `unicode-range`, перенесёнными из ответа
  Google дословно. Скрипт падает, если семейство перестало быть вариативным:
  схема «один файл на срез» тогда молча теряла бы начертания.
- CJK на свой хост не забирается и с Google больше не грузится: стек системных
  наборов по `html[lang]`. PingFang SC, Hiragino Sans, Apple SD Gothic Neo,
  Noto Sans CJK на Android стоят на этих рынках повсеместно и отрисованы под
  свои письменности лучше, чем веб-шрифт, приехавший вторым запросом.
- Заодно чинится `font-extrabold` (800): в запросе стояли 400/500/600/700, и 800
  браузер рисовал синтетическим утолщением. У вариативного файла
  `font-weight: 100 900` — начертание настоящее.
- `inlineStylesheets` вернулся в `"always"`. Условие, записанное прошлым раундом
  («только вместе с self-host шрифтов»), выполнено, и арифметика перевернулась:
  было 11 882 B HTML + 6 843 B CSS = 18 725 B в два запроса, стало 18 494 B в
  один.
- `checkFontDelivery()` в `check-seo` роняет сборку на любом обращении к
  fonts.googleapis.com / fonts.gstatic.com, на `preload` шрифта без `@font-face`
  на той же странице и на файле в `public/fonts/`, который никто не объявляет.
  Проверено подсунутым `<link>`: чекер падает с обоими сообщениями.

Проверено в браузере на собранном сайте: на `/zh/`, `/ja/`, `/th/`, `/ar/`,
`/ru/` в сетевой панели ноль запросов к Google, один `preload` латинского среза,
CJK, тайская вязь и арабица рисуются корректно.

### 12.2 Пять языков перестали быть витриной

Замер по сайтмапу: en 56 URL, ru 56, th/ar/zh/ko/ja — по 15. Каталогов
`questions/` и `getting-here/` на этих локалях не существовало вовсе: не
`noindex`, а 404. При этом `locales` у самих вопросов в `question-pages.ts` УЖЕ
объявляли эти аудитории — решение «вопрос сюда относится» было принято, а текста
под него никто не написал, и `getQuestionPage()` возвращал `null`. Ограничением
были данные, а не ворота: завод отдавал 87 кандидатов при 0 отказов.

Написано `src/data/question-pages-open-locales.ts` — девять новых страниц:
`questions/rules-and-prescription` на th/ar/zh/ko/ja и `questions/taking-it-home`
на ar/zh/ko/ja. Тайской страницы про вывоз нет намеренно: для тайского читателя
этот вопрос не стоит, и это тот же принцип, по которому локали объявляются у
вопроса, а не у страницы.

Текст написан, а не переведён. Английская страница про вывоз обращается к
читателю, чья страна на это в основном пожмёт плечами; японская и корейская —
про ровно обратное (поправка 2023 года криминализовала употребление, и обе
страны применяют свой закон к гражданам за границей), и это разный текст с
первого абзаца. Все девять прошли ворота с запасом: th 5 219 симв. при пороге
2 500, zh 1 649–1 808 при 1 050, ko 2 265–2 393 при 1 300, ja 2 431–2 545 при
1 450, ar 962–996 слов при 640.

Оверлей вливается в `QUESTION_PAGES` при загрузке модуля, а не в момент сборки
страницы, — иначе `assertBasisLabelHonesty()` не увидела бы новый текст, и
единственный машинный контроль честности ярлыков молча перестал бы работать
ровно на непроверенном человеком материале. Слияние дополнительно падает, если
оверлей даёт текст на локали, которую вопрос не объявил.

Индексируемых URL: 187 → **196** при потолке 200, заводских 87 → 96 при потолке
100. Потолки не поднимались.

### 12.3 Внутренний граф

- **Скрытый обрез курируемых соседей.** `getRelatedLinks()` резал
  `slice(0, limit)` по всему набору. Главная объявляет пятнадцать соседей при
  `limit={12}` — три последних не рисовались, и среди них `strains`, хаб
  кластера, под которым 29% индексируемого набора. Тот же обрез на одну ссылку
  был у `cannabis-near-me-pattaya`. Теперь `limit` — потолок ДОБОРА, а не
  списка: своё выводится целиком.
- **Сквозная навигация к кластерам.** Замер глубины клика от главной:
  {1: 19, 2: 90, 3: 57, 4: 20} — 77 из 187 URL в трёх и более кликах. Шапка даёт
  две настоящие ссылки, футер давал три. `strains` и `guides` добавлены в
  служебный футер — не в шапку: она скрыта до 768px, а Google индексирует
  мобильной версией.
- **Симметрия сортов.** `compareWith` читался только вперёд, и Pineapple Express,
  которого не называет ни один другой сорт, имел одну входящую ссылку — с хаба —
  и самый низкий вес во всём наборе. Обратный индекс даёт 70 связей вместо 60:
  у двенадцати сортов из двадцати не меняется ничего, починка достаётся тем
  восьми, кому её и не хватало.
- Разброс источников на indexable-страницу: 1–51 → **1–42**.

### 12.4 Сущность в разметке — один узел вместо трёх

`#business` (LocalBusiness+Store) и `#org` (Organization) были неразличимы —
то же имя, телефон, адрес, `sameAs`, `contactPoint`, — и первый объявлял
`parentOrganization: {"@id": "#org"}`: утверждение, что магазин является дочерним
предприятием компании, у которой случайно совпадают название, улица и телефон.
Такой компании нет. Третьим кандидатом на ту же сущность был анонимный
`author: {"@type":"Organization","name":"Labs Cannabis"}` на 101 странице статьи.

Отдельно: `#business` публиковался с `url` = `/en/` на английских страницах,
`/ru/` на русских, `/th/` на тайских — один `@id`, семь разных адресов.

Стало: один узел `["Organization","LocalBusiness","Store"]` с одним `url`;
`author` и `publisher` статьи — ссылки по `@id` на него же. Для сайта, весь
смысл страницы `about` которого в том, чтобы сущность была одна и не склеивалась
с чужим магазином на соседней улице, три кандидата в графе — не мелочь.

Добавлен `ItemList`: три суффикса типизированы как `CollectionPage`, и во всей
сборке было 0 узлов `ItemList` при 21 объявленном `CollectionPage` — страница
говорила «я список» и не перечисляла ничего. Обещания карусели здесь нет и быть
не может (Google собирает их для Course/Movie/Recipe/Restaurant/Product), смысл
в другом: состав хаба стал машиночитаемым в том же виде, в каком его видит
человек. На `/locations/` список подвешен через `hasPart` — `mainEntity` там
занят самим бизнесом, а два `mainEntity` на узле это одно противоречивое
утверждение, а не два.

### 12.5 Сниппет

- **Обрыв посреди слова.** `seoDescription()` резал по счётчику символов и
  дописывал `...`. Ровно восемь описаний заканчивались многоточием, и это были
  восемь самых коммерческих URL набора: `buy-`, `best-`, `cheap-` и брендовая
  страница на en и ru («…Adults 20+ who hold a Thai prescripti...», «…живую
  карточку с п...»). Рез теперь идёт по границе предложения, иначе слова, и без
  многоточия; граница ищется и в восточноазиатской пунктуации. Многоточие в
  конце описания — блокирующая ошибка чекера.
- **Длина.** `MAX_TITLE_LENGTH` 75 → 65 (пять заголовков сокращены), рядом —
  отчёт `reportSerpWidth()` о том, что вылезает за 60/160. Отчётом, а не
  падением: Google подменяет авторское описание собственным пассажем на
  большинстве запросов, и красная сборка за упущенную возможность обесценивает
  красную сборку за поломку. Заголовков длиннее 60: 24 → 18.

### 12.6 Сайтмап разбит по кластерам

Search Console показывает покрытие в разрезе ОТПРАВЛЕННОГО сайтмапа, и одна
плоская карта давала одну строку отчёта на весь сайт. Вердикт ворот качества при
этом принимался вслепую: 149 отказов «Обнаружена, не проиндексирована», ради
которых ворота и строились, не разложены ни по кластеру, ни по локали.

`scripts/build-sitemaps.mjs` (шаг `npm run build`) режет вывод по границе «кто
допустил URL»: `sitemap-pages.xml` 93, `sitemap-strains.xml` 62,
`sitemap-questions.xml` 25, `sitemap-getting-here.xml` 16. Ни один URL, ни один
`lastmod`, ни одна альтернатива `xhtml:link` не изменены — блоки переносятся
дословно. Детерминизм проверен: две сборки подряд дают побайтово одинаковые
карты. `robots.txt` по-прежнему указывает на `sitemap-index.xml`, IndexNow уже
читал дочерние карты из индекса.

**ownerTask после деплоя:** отправить четыре новые карты в Search Console и
Яндекс.Вебмастер и удалить оттуда старую запись `sitemap-0.xml`.

### 12.7 Уокинг Стрит на семи языках

Единственный ориентир набора, у которого имя стояло латиницей на всех семи
локалях — остальные двенадцать локализованы. Строка печатается на каждой
странице через `describeLandmarkWalk()`, поэтому замер прямой: кириллического
«Уокинг» не было ни на одной из 67 русских страниц, тайского «ถนนคนเดิน» — ни на
одной из 30 тайских, а китайская страница доставки выводила
«Walking Street：800 米». Заполнено в `landmarks.ts` и в `seo-matrix.ts`; в
русской странице района кириллическая форма добавлена в H1 и описание, а
`<title>` оставлен латинским — удваивать одно название в заголовке выдачи значит
потратить видимую половину на повтор.

### 12.8 Что выяснилось про прод и требует владельца

Из среды разработки внешний egress РАБОТАЕТ — прежние планы считали его
заблокированным, — поэтому O-08 закрыт замером:

| URL | Ответ |
|---|---|
| `/` | 308 → `/en/` |
| `/en/locations` | 308 → `/en/locations/` |
| `/en/wholesale/` | 308 → `/en/locations/` |
| `/review` | 307 → форма отзыва Google |
| `www.labscannabis.boutique` | 307 → апекс `.boutique` |
| **`labscannabis.com`** | **404, отдаёт Wix за Cloudflare** |

Внутри `.boutique` цепочек нет, всё как задумано.

**Про `labscannabis.com` — поправка от 31.08.2026, в тот же день.** Первая
редакция этого раздела записала домен как «наш, но не подключённый к Vercel», и
завела ownerTask «подключить и направить DNS». Это неверно: **домен нам не
принадлежит и не покупался**. Он отвечает 404 с сайта на Wix за Cloudflare —
то есть занят кем-то другим. Два правила с `has: host` в `vercel.json` не просто
не срабатывали: они утверждали в коде, что домен наш, и именно поэтому их никто
не оспорил за несколько аудитов подряд. Правила удалены, раздел «Redirects» в
`README.md` переписан. Никакого исторического веса на этом домене у проекта нет
и не было — терять там нечего.

Что из этого следует: единственный домен проекта — `labscannabis.boutique`, и
весь внешний вес должен идти на него. Если владелец захочет `.com`, это покупка
домена у текущего владельца, а не настройка DNS, и до покупки никаких правил в
`vercel.json` быть не должно.

### 12.9 Отправлено в панели вебмастера 31.08.2026

**Google Search Console — сделано.** Ресурс заведён как **URL-префикс**
`https://labscannabis.boutique/`, а не как домен `sc-domain:` — из-за неверно
угаданного `resource_id` первая попытка получила «У вас нет доступа к этому
ресурсу», и это было записано в план как отсутствие доступа. Доступ есть.
Отправлены и уже обработаны со статусом «Успешно»:

| Файл | Выявлено страниц |
|---|---|
| `sitemap-pages.xml` | 93 |
| `sitemap-strains.xml` | 62 |
| `sitemap-questions.xml` | 25 |
| `sitemap-getting-here.xml` | 16 |

Сумма 196 совпадает с `EXPECTED_INDEXABLE_PAGE_COUNT`. Индекс
`sitemap-index.xml` переотправлен, обработан 31.08; в колонке «выявлено
страниц» у него пока висит прежняя 41 — GSC обновляет эту цифру у индексных
файлов с задержкой, а содержательные строки теперь дочерние. Строки
`sitemap-0.xml` в панели не было вовсе, удалять нечего.

Теперь покрытие в отчёте «Страницы» фильтруется по каждому кластеру
отдельно — ради этого разбиение и делалось: у вердикта ворот качества появилась
обратная связь.

**Яндекс.Вебмастер — сделано.** Запрошен переобход `sitemap-index.xml` (счётчик
10 → 9), чтобы Яндекс перечитал индекс и подобрал четыре новые карты. Отдельно
отправлены на переобход 17 URL: девять новых вопросных страниц на th/ar/zh/ko/ja
и восемь изменившихся русских. Дневной лимит — 150.

### 12.10 Telegram и LINE включены

Оба канала включены 31.08.2026 по подтверждению владельца: аккаунты заведены на
том же номере, что и WhatsApp, и ссылка на Telegram открывает у него приложение
с нужным аккаунтом. `telegramEnabled: true`, `lineEnabled: true`.

**Где проходит граница проверяемого.** Ни одну из двух ссылок нельзя
подтвердить с этой машины, и по одинаковой причине: обе разрешаются внутри
приложения, а не на сервере.

- Telegram отдаёт настоящую контактную страницу («Chat with +66 6 6080 6784» →
  OPEN CHAT), но чужое приложение разрешит номер, только пока у аккаунта
  «Кто может найти меня по номеру телефона» = «Все».
- LINE на десктопе рисует QR и не проверяет токен; резолвит его только телефон.

Значит настоящая проверка ровно одна и делается за минуту: открыть обе кнопки
**с телефона, где магазин не сохранён в контактах**. Если канал мёртв — это одно
слово в `src/data/site.ts` (`lineEnabled` / `telegramEnabled` в `false`).

**Поправка к предыдущей редакции этого раздела.** Здесь стояло, что LINE
включать нельзя, и в доказательство приводился «контрольный опыт»: страница
якобы отдаёт побайтово одинаковый экран для нашего токена, для
`zzzz-not-a-real-line-id-9999` и для `~labscannabis`. **Замер был неверен** —
сравнивался base64 QR-картинки, обрезанный до 160 символов, то есть только
PNG-заголовок. При сравнении целиком картинки разные: 674 / 962 / 854 / 878
байт, разные хеши. Страница кодирует в QR именно тот токен, который ей дали.
Одинаков текст, а не QR, и вывод «QR ничего не доказывает» из этого не следует.

Что остаётся верным и без того замера: документированные формы этого эндпоинта —
`~<lineId>`, `@<официальный аккаунт>` и короткая `lin.ee/XXXX`, национальный
номер среди них не значится. Но это довод «в документации такого нет», а не
«проверено, что не работает», и против теста владельца на живом телефоне он не
весит. Если ссылка окажется мёртвой, правильная замена — LINE ID с включённым
поиском по ID (`line.me/ti/p/~<ID>`) или Official Account (`lin.ee/XXXX`):
последний ещё и не привязан к личному аккаунту и переживает смену человека за
прилавком.

**Остаточный дефект Telegram, не влияющий на кнопку.** `t.me/+<цифры>`
двусмысленна: Telegram читает `+` сначала как хеш приватного приглашения,
поэтому серверное превью ссылки — `og:title = "Join group chat on Telegram"` с
пустым описанием. Это видит тот, кому нашу ссылку переслали в мессенджере.
`@username` снял бы и это, и зависимость от настройки приватности.
