...
export const COMPLIANCE_RULES = Object.freeze([
  ...
  // --- Обесценивание вывески ------------------------------------------------
  // У заведения два действующих названия. LABS DISPENSARY стоит на вывеске и в
  // карточке Google, под ним магазин знают годами, и брендовый спрос идёт
  // в основном на него. Любое «ранее / бывшее /旧称» объявляет это имя мёртвым:
  // человек, пришедший по брендовому запросу, читает, что искал не то, а Google
  // получает повод отдать бренд однофамильцам (GANJ LABS, White Labs, Green Lab).
  rule(
    "former-name-en",
    /\b(?:formerly|previously|used to be|was) (?:known as |listed as |called |named )?(?:labs )?dispensary\b|\b(?:former|old|previous|earlier) name\b|\bunder its old name\b/gi,
    "название на вывеске подано как устаревшее",
  ),
  rule(
    "former-name-ru",
    /(?<!\p{L})(?:ранее|раньше|прежде)\s+(?:извест\p{L}*|называ\p{L}*|это\s+был\p{L}*)?\s*(?:как\s+)?labs\s+dispensary|(?<!\p{L})(?:бывш\p{L}+|прежн\p{L}+|старо\p{L}+)\s+(?:назван\p{L}+|имя)/giu,
    "название на вывеске подано как устаревшее",
  ),
  rule("former-name-cjk", /旧称|旧名|以前は\s*Labs\s*Dispensary|이전\s*(?:이름|명칭)|此前是|曾用名/g, "название на вывеске подано как устаревшее"),
