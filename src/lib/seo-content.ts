...
/**
 * `content-cache` — 42 файла (6 слагов × 7 локалей), которые до W1-11 не читала
 * ни одна строка `src/`: их открывали только генераторы. Страницы отдавали
 * ручную копию на 60-240 слов, а оплаченный текст на 250-550 слов в сборку не
 * попадал.
 *
 * Файлов было 168 (24 слага). 126 из них — 18 слагов — не рендерились ни одной
 * страницей: `SEO_PAGES` успели сократить, а кэш остался от прежнего, широкого
 * списка. Мёртвого текста в живом каталоге больше нет, и вернуться он не может:
 * `checkOrphanContentCache()` в `scripts/check-seo.mjs` валит сборку на любом
 * файле кэша, за которым нет собранной страницы. Обоснование удаления — в
 * `docs/growth/ops/05-content-cache-orphans.md`.
 *
 * Читаем ровно как `src/lib/reviews.ts`: `node:fs` на этапе SSG. Ни
 * `import.meta.glob`, ни `fetch` — иначе JSON уедут в клиентский бандл.
 */
